...
with open('./test_data/yelp_academic_dataset_review.json') as f:
    for line in f:
        ...
        stars = data['stars']
        if(stars > 3 and positive_train_len<15000):
            train_positive[positive_train_len] = review_data
            positive_train_len += 1
        elif(stars <= 3 and negative_train_len<15000):
            ...
            negative_test_len += 1
                  ...
                  positive_test_len, negative_test_len])<60000):
            continue
        else:
            break
...
for folder in folder_list:
    ...
    epoch = 0
    model = torch.load(f"../weights/{folder}/{epoch}epochs_model.pt", map_location=device)
    vocab = torch.load(f"../weights/{folder}/vocab.pt")
